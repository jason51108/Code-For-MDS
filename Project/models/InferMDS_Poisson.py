...
def Log_likelihood(A, M):
    result = np.sum(A*M - np.exp(M) - np.log(factorial(A)))
    diagonal_correction = np.sum(np.diag(A*M - np.exp(M) - np.log(factorial(A))))
...
if __name__ == '__main__':
    ...
    if not bool(os.listdir(path_ )):
        ...
        np.save(f'/home/user/CYH/Code_For_MDS/para_result/Poisson/{setting}/n_{num_samples}/{number}/adjacency_matrix.npy', result[4])
